...
class MultichainClient
{
    ...
    /**
     * Returns the private key corresponding to the specified address.
     *
     * @param bool $address
     * @return string
     */
    ...
    /**
     * Returns a list of transactions for the asset.
     *
     ...
     * @return mixed
     */
    public function listAssetTransactions($assetName, $verbose = true, $count = 10, $start = -10, $localOrdering = false)
    {
        $start = -$count;
        return $this->jsonRPCClient->execute("listassettransactions", array($assetName, $verbose, $count, $start, $localOrdering));
    }
     ...
     * @return mixed
     */
    public function grant($addresses, $permissions, $nativeAmount = 0, $startBlock = 0, $endBlock = null, $comment = '', $commentTo = '')
    {
    ...
    {
        $params = array($fromAddress, $toAddress, $name, $qty, $units, $nativeAmount);
        if (!is_null($custom)) {
            $params[] = $custom;
        }
        return $this->jsonRPCClient->execute("issuefrom", $params);
    }

    /**
     * Issuing more quantity of a given asset.
     *
     * @param $toAddress
     * @param $name
     * @param $qty
     ...
     * @return mixed
     */
    public function issueMore($toAddress, $name, $qty, $nativeAmount = 0, $custom = null)
    {
        $params = array($toAddress, $name, $qty, $nativeAmount);
        if (!is_null($custom)) {
            $params[] = $custom;
        }
     ...
     * @return mixed
     */
    public function issueMoreFrom($fromAddress, $toAddress, $name, $qty, $nativeAmount = 0, $custom = null)
    {
        $params = array($fromAddress, $toAddress, $name, $qty, $nativeAmount);
        if (!is_null($custom)) {
            $params[] = $custom;
        }
    ...
    /**
     * Locks/Unlocks a transaction output.
     *
     * @param bool $unlock
     * @return mixed $transactions
     */
    public function lockUnspent($unlock, $transactions)
    {
        return $this->jsonRPCClient->execute("lockunspent", array($unlock, $transactions));
    }
    ...
    {
        return $this->jsonRPCClient->execute("getblockcount");
    }
    ...
    /**
     * Returns a list of locked unspent transaction outputs in the wallet, with between minconf and maxconf confirmations. For
     * a MultiChain blockchain, each transaction output includes assets and permissions fields listing any assets or
     * permission changes encoded within that output. If addresses is provided, only outputs which pay an address in
     * this array will be included.
     *
    ...
    {
        return $this->jsonRPCClient->execute("listlockunspent");
    }
    ...
    /**
     * Subscribes the node to an asset or stream.
     *
     * @param $entityName
     */    
    public function subscribe($entityName, $rescan=true)
    {
        return $this->jsonRPCClient->execute("subscribe", array($entityName, $rescan));
    }
    ...
    public function getStreamItem($streamID, $txID, $verbose = true)
    {
        return $this->jsonRPCClient->execute("getstreamitem", array($streamID, $txID, $verbose));
    }
