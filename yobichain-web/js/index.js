...
var sendHashToBlockchain = function (hash) {
			if (hash.length == 0) { 
        ...
        var act = document.getElementById("action").value;
				...
				xmlhttp.onreadystatechange = function() {
					if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
						document.getElementById("bcResult").innerHTML = xmlhttp.responseText;
					}
				};
        document.getElementById("bcResult").innerHTML = "";
				xmlhttp.open("GET", "processor.php?hash=" + hash + "&action=" + act, true);
				xmlhttp.send();
			}
		}
...
$(document).ready(function() {
  ...
  var handleFileSelect = function(f) {
    ...
    var output = '';
    output = translate('Preparing to verify ') + escape(f.name) + ' (' + (f.type || translate('n/a')) + ') - ' + f.size + translate(' bytes, last modified: ') + (f.lastModifiedDate ? f.lastModifiedDate
  ...
  var crypto_finish = function(hash) {
    bar.width(100 + '%');
    explain.html(translate('File hash: ') + hash);
